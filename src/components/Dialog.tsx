...
export const DialogContent = ({ className, children, ...props }: React.ComponentProps<typeof Content>) => {
  return (
    <DialogPortal>
      ...
      <Content
        className={cn(
          "fixed top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-xl bg-body p-6 shadow data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=closed]:slide-out-to-top-4 data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 data-[state=open]:slide-in-from-top-4 sm:max-w-lg sm:border sm:border-foreground/15 sm:data-[state=closed]:animate-out sm:data-[state=open]:animate-in",
          className,
        )}
        {...props}
      >
